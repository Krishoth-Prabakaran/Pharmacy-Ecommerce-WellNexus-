...
class _PharmacyRegisterScreenState extends State<PharmacyRegisterScreen> {
  ...
  Future<void> _registerPharmacy() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (!hasValidBranch) {
        _showSnackBar('At least one branch must have address and phone', Colors.red);
        return;
      }
  ...
  void _setMainBranch(int index) {
    setState(() {
      for (int i = 0; i < _branches.length; i++) {
        ...
        if (i == index) {
          _branches[i]['branch_name'] = 'Main Branch';
        }
      }
    });
  }
  ...
  Future<void> _useCurrentLocation(int branchIndex) async {
    try {
      setState(() {
        _locationMessage = 'Finding current location...';
      });
      final position = await LocationService.getCurrentLocation();
      _updateSelectedLocation(LatLng(position.latitude, position.longitude), branchIndex);
    } catch (error) {
      _showSnackBar(error.toString(), Colors.red);
      setState(() {
        _locationMessage = 'Unable to get current location';
      });
    }
  }
